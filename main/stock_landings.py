...
def main():
    ...
    species_landings["Location Match"] = species_landings["Location"].apply(extract_alphanumeric)
    weights["Location Match"] = weights["Location"].apply(extract_alphanumeric)
    
    stock_landings = pd.merge(species_landings, 
                              weights, 
                              on=["Area", "ASFIS Scientific Name", "Location Match"],
                              how="inner",
                              suffixes=("", "_x"))
    stock_landings = stock_landings.rename(columns={2021: "Species Landings 2021"})
    cols_to_keep = ["Area", "ASFIS Scientific Name", "Location", "Species Landings 2021", "Normalized Weight"]
    ...
    proxy_landings = pd.read_excel(os.path.join(input_dir, "January overview - one table.xlsx"), 
    ...
    proxy_landings = proxy_landings.drop_duplicates(["Area", "ASFIS Scientific Name", "Location"])
    proxy_cols = ["Area", "ASFIS Scientific Name", "Location", "Proxy Species", "Proxy Landings"]
    ...
    stock_assessments = stock_assessments[primary_key + ["ISSCAAP Code", "ASFIS Name", "Status"]]
    ...
    isscaap_to_nei = {int(k): v for k,v in isscaap_to_nei.items()} #JSON saves keys are strings
    ...
    numerical_areas = [area for area in stock_landings["Area"].unique() if isinstance(area, int)]
    stock_landings = compute_missing_landings(stock_landings, fishstat, numerical_areas, isscaap_to_nei)
    ...
    stock_landings.to_excel(os.path.join(output_dir, "stock_landings.xlsx"), index=False)
