...
def main():
    ...
    stock_assessments_full = pd.read_excel(os.path.join(clean_data_dir, "stock_assessments_w_unassessed.xlsx"))
    stock_assessments = pd.read_excel(os.path.join(clean_data_dir, "stock_assessments.xlsx"))
    ...
    sbn_sofia_dict = read_stock_data(sofia_file_path, sofia_indices, desc="SOFIA Sheets")

    # Reformat SOFIA status by number
    for sheet, df in sbn_sofia_dict.items():
        sbn_sofia_dict[sheet]["Area"] = sofia_sheet_to_area[sheet]
        sbn_sofia_dict[sheet] = df[["Area", "Overfished", "Fully Fished ", "Under fished"]]
        ...
        sbn_sofia_dict[sheet]["No. of Sustainable"] = sbn_sofia_dict[sheet]["No. of U"] + \
                                                    sbn_sofia_dict[sheet]["No. of MSF"]
        sbn_sofia_dict[sheet]["No. of Unsustainable"] = sbn_sofia_dict[sheet]["No. of O"]
        sbn_sofia_dict[sheet]["No. of stocks"] = sbn_sofia_dict[sheet]["No. of Sustainable"] + \
    ...
    sbn_sofia = pd.concat([sbn_sofia, 
                        pd.DataFrame({"Area": "Global"}, index=[len(sbn_sofia)])])

    cols_to_sum = ["No. of stocks", "No. of U", "No. of MSF", "No. of O", "No. of Sustainable", "No. of Unsustainable"]
    sbn_sofia.loc[sbn_sofia["Area"]=="Global", cols_to_sum] = sbn_sofia[cols_to_sum].sum().values
    ...
    for area in stock_assessments["Area"].unique():
        ...
        area_summary_fp = os.path.join(output_dir, os.path.join("Area Statistics", f"area_{area}_summary.xlsx"))
    ...
    sbn_top10_species = compute_species_status_by_number(stock_assessments, top_ten_species, fishstat)
    ...
    fishstat["ISSCAAP Code"] = fishstat["ASFIS Scientific Name"].map(scientific_to_isscaap)
    ...
    isscaap_code_to_name = dict(zip(asfis["ISSCAAP_Group_Code"], asfis["ISSCAAP_Group_Name_EN"]))
    
    # Retrieve country data and create ISO3 to name map
    country_codes = pd.read_excel(os.path.join(input_dir, "NOCS.xlsx"), sheet_name="Codes")
    ...
    pc = compute_percent_coverage(stock_landings, 
                                                fishstat, areas, 
                                                assessment="Update",
    ...
    pc_sofia = compute_percent_coverage(sofia_landings,
                                        fishstat, areas,
                                        assessment="Previous",
                                        key="Proxy",
                                        landings_key=2021,
    ...
    wp_wo_tuna = compute_weighted_percentages(stock_landings, fishstat=fishstat, tuna_location_to_area=location_to_area["Tuna"])
    ...
    sofia_landings_assessed = sofia_landings_assessed.rename(columns={2021: "Stock Landings 2021"})
    sofia_landings_assessed = sofia_landings_assessed[["Area", "ASFIS Scientific Name", "Status", "Stock Landings 2021"]]
    ...
    wp_totl_wo_tuna = get_weighted_percentages_and_total_landings(wp_wo_tuna, appendix_decs)
    wp_totl_wo_tuna_fp = os.path.join(output_dir, "status_by_landings_w_totals_wo_tuna.xlsx")
    add_footnote(wp_totl_wo_tuna, sbl_footnote, multi_index=True).to_excel(wp_totl_wo_tuna_fp)
