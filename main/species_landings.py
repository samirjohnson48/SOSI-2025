...
def main():
    ...
    species_landings[years] = species_landings.apply(
        ...
        args=(fishstat, location_to_area),
        axis=1
    )
    ...
    sardinella_landings = fishstat[sardinella_spp_mask & (fishstat["Area"] == 47)][years].sum()
    
    sardinella_mask = species_landings["ASFIS Scientific Name"].apply(lambda x: "Sardinella" in x)
    area_47_mask = species_landings["Area"] == 47
    n_sardinella = sum(sardinella_mask & area_47_mask)
    
    species_landings.loc[sardinella_mask&area_47_mask, years] = (sardinella_landings / n_sardinella).values
    ...
    sebastes_area_mask = fishstat["Area"].isin(location_to_area["Deep Sea"]["Divisions 3LN Grand Bank"])
    sebastes_landings = fishstat[sebastes_spp_mask & sebastes_area_mask][years].sum()
    
    sebastes_mask = species_landings["ASFIS Scientific Name"] == "Sebastes mentella, Sebastes fasciatus"
    deep_sea_mask = species_landings["Area"] == "Deep Sea"
    
    species_landings.loc[sebastes_mask&deep_sea_mask, years] = sebastes_landings.values
