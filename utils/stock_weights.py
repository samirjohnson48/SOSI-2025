...
def retrieve_31_37_81_weights(file_path, sheet_names=["Area31Jeremy", "37", "81"]):
    ...
    for sheet in sheet_names:
        ...
        rename_dict = {
            ...
            "Landings": "Weight 1",
            "Landings (tonnes) Author's Observation / Estimate (2021) - if not in FishStat":
                "Weight 1",
        ...
        data["ASFIS Scientific Name"] = data[["ASFIS Scientific Name", "MAASN"]].apply(
                                                  lambda row: row["MAASN"] if row["MAASN"]!="to ignore" else row["ASFIS Scientific Name"],
                                                  axis=1
                                              )
        ...
        if sheet == "81":
            df = data[
                data["Weight 1"].apply(lambda x: isinstance(x, str) and "notes" in x.lower())
            ...
            df.loc[
                df["Location"] == "SNA 1 (East Northland)", "Weight 1"
            ] = 2289.5
            
            # Update Area 81 weights with the weights taken from the notes
            df = df[["ASFIS Scientific Name", "Location", "Weight 1"]]
            data = pd.merge(data, df, on=["ASFIS Scientific Name", "Location"], how="left", suffixes=("", " Update"))
...
def merge_weights(weights, new_weights, primary_key, weight1_na=False, clean_location=False):
    if clean_location:
        weights["Location Match"] = weights["Location"].apply(extract_alphanumeric)
        new_weights["Location Match"] = new_weights["Location"].apply(extract_alphanumeric)
        primary_key.remove("Location")
        primary_key.append("Location Match")
        
    merge = pd.merge(weights, new_weights, on=primary_key, how="left", suffixes=("", "_new"), indicator=True)
...
def validate_normalization(
    weights, 
    group_key=["Area", "ASFIS Scientific Name"], 
    weight_key="Normalized Weight"
    ):
    for group_name, group in weights.groupby(group_key):
        assert np.isclose(group[weight_key].sum(), 1), f"Weights for {group_name} are not normalized"
