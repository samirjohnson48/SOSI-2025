...
def compute_landings(row, species_landings="Species Landings 2021", weight="Normalized Weight"):
    if row["Area"] == "Tuna":
...
def use_proxy_landings(stock_landings, 
                       proxy_landings, 
                       primary_key=["Area", "ASFIS Scientific Name", "Location"]):
    merge = pd.merge(stock_landings, proxy_landings, on=primary_key, how="left")
    
    no_landings_mask = merge["Stock Landings 2021"].isna() | merge["Stock Landings 2021"] == 0
    merge.loc[no_landings_mask, "Stock Landings 2021"] = [float(val) for val in merge.loc[no_landings_mask, "Proxy Landings"].values]
    merge.loc[~no_landings_mask, "Proxy Species"] = np.nan
    
    return merge

def compute_missing_landings(stock_landings, fishstat, areas, isscaap_to_nei, year=2021):
    sl = stock_landings.copy()
    ...
    for area in areas:
        ...
        no_landings_mask = (df["Stock Landings 2021"] == 0) | (df["Stock Landings 2021"].isna()) 
        if sum(no_landings_mask) == 0:
            continue
        
        # Get the list of NEI species whose landings will be used
        proxy_species = df[no_landings_mask]["ISSCAAP Code"].map(isscaap_to_nei).unique()
                        
        # Don't include seals since they are taken out in calculations
        seals_mask = df["ISSCAAP Code"] == 63
        no_l = df[(no_landings_mask | (df["ASFIS Name"].isin(proxy_species))) & ~seals_mask]
        ...
        u, m, o = no_l_counts.get("U", 0), no_l_counts.get("M", 0), no_l_counts.get("O", 0)
        
        # Assign stock landings for stocks with no landings 
        # (or reassign landings for Marine Fishes NEI so not to double count) 
        u_mask, m_mask, o_mask = sl["Status"] == "U", sl["Status"] == "M", sl["Status"] == "O"
        no_l_mask = (sl["Stock Landings 2021"]==0) | (sl["Stock Landings 2021"].isna())
        nei_mask = sl["ASFIS Name"].isin(proxy_species)
        no_seals_mask = ~(sl["ISSCAAP Code"] == 63)
        
        if u > 0:
            sl.loc[(area_mask)&(u_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Stock Landings 2021"] = u_l / u
            sl.loc[(area_mask)&(u_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Proxy Species"] = "NEI"
        if m > 0:
            sl.loc[(area_mask)&(m_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Stock Landings 2021"] = m_l / m
            sl.loc[(area_mask)&(m_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Proxy Species"] = "NEI"
        if o > 0:
            sl.loc[(area_mask)&(o_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Stock Landings 2021"] = o_l / o
            sl.loc[(area_mask)&(o_mask)&(no_l_mask | nei_mask)&(no_seals_mask), "Proxy Species"] = "NEI"
